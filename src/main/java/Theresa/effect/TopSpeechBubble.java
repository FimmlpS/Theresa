...
public class TopSpeechBubble extends AbstractGameEffect {
    ...
    public TopSpeechBubble(float x, float y, String msg, boolean isPlayer) {
        this(x, y, 2.0F, msg, isPlayer);
    }

    public TopSpeechBubble(float x, float y, float duration, String msg, boolean isPlayer) {
        ...
        AbstractDungeon.topLevelEffectsQueue.add(new SpeechTextEffect(x + effect_x, y + 124.0F * Settings.scale, duration, msg, DialogWord.AppearEffect.BUMP_IN));
        if (isPlayer) {
            this.x = x + ADJUST_X;
        } else {
            this.x = x - ADJUST_X;
